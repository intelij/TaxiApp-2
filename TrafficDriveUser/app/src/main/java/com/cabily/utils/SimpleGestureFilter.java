...
/**
 * Created by user88 on 6/29/2017.
 */
...
public class SimpleGestureFilter extends GestureDetector.SimpleOnGestureListener {
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
        ...
        if (Math.abs(distanceX) > Math.abs(distanceY) && Math.abs(distanceX) > this.swipe_Min_Distance && Math.abs(velocityX) > this.swipe_Min_Velocity) {
            if (distanceX > 0)
